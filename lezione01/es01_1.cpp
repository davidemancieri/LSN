...
int main (){
	...
	for( int i=0; i<N; i++)
		out << (x[i]+1) << " " << sum_prog[i] << " " << err_prog[i] << endl;
	out.close();
	...
	M = 1E4;	
	int sub = 100;
	vector<double> ni(sub,0);
	vector<double> chij(100,0);
	...
	for(int i=0; i<100; i++){
		...
		int k=0;
		for(int j=0; j<sub; j++)
			while(r[k]<((j+1.)/sub) and k<M){	//k is used to not waste time checking values already
				ni[j]++;			//put into a sub-interval
				k++;
			}
		
		for(int l=0; l<sub;l++)
			chij[i]+=pow((ni[l])-(M/sub), 2)/(M/sub);
	}	
