...
double variationalWaveFunction_second(double v)
{
	...
	double mu = 0.805;
	double sigma = 0.61;
	double e_neg = exp(-pow(v-mu,2.)/(2.*sigma*sigma))*(pow((v-mu)/(sigma*sigma),2.)-(1./(sigma*sigma)));
	double e_pos = exp(-pow(v+mu,2.)/(2.*sigma*sigma))*(pow((v+mu)/(sigma*sigma),2.)-(1./(sigma*sigma)));
	return e_neg + e_pos;
}
