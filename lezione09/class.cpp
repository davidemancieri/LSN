...
vector<int> Generation :: block_permutation( vector<int> path ){
	...
	int m = m_rnd.Rannyu( 1, max+1 );			//m can't be higher than max because we need two different blocks of cities of length m to move
...
void Generation :: stats( string output ){
	...
	double sum=0.;
	int k=0;
	for( int i=0; i<m_Nindividuals/2; i++){				//averaging the path length of the better half of the individuals
		sum += m_individuals[i].getL1();
		k++;
	}
	sum = sum/k;
	fitnessL1 << m_individuals[0].getL1() << "   " << sum << endl;	//printing the length of the best path and the averaged one
	fitnessL1.close();
